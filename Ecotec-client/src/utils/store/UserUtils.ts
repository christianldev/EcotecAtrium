...
import {
	...
	setLogged,
	setRole,
} from '@/store/modules/accountReducer';
...
import {
	...
	setCurrent,
	resetTenantState,
} from '@/store/modules/tenantReducer';
...
const logged = (
	response: UserLoggedResponse,
	navigate: NavigateFunction
) => {
	console.log('logged', response);
	store.dispatch(login(response));
	const role = response.user.role.includes('admin')
		? UserType.Admin
		: UserType.Student;
	...
	if (redirect) {
		...
		if (
			...
			import.meta.env.VITE_REACT_APP_SERVICE !== 'sandbox'
		) {
			navigate('/admin');
		} else {
