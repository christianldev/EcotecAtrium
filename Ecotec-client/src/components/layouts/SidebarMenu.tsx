        ...
        {getMenu.map((group, index) => {
          return (
            <div key={index} className="mt-2">
              <div className="mt-2">
                <h3 className="px-1 text-xs leading-4 font-semibold text-slate-500 uppercase tracking-wider">
                  {group.title}
                </h3>
              </div>
                          ...
                          <div>
                            ...
                            >
                              {menuItem.icon !== undefined && (
                                <SidebarIcon className="h-5 w-5 text-white" icon={menuItem.icon} />
                              )}
            ...
            <div key={index} className="select-none">
              <div className="mt-2">
                <h3
                  ...
                  className="px-1 text-xs leading-4 font-semibold text-slate-500 uppercase tracking-wider"
                >
                  {group.title}
                </h3>
              </div>
                        ...
                        return (
                          <div>
                            <Link
                              to={menuItem.path}
                              className={classNames(
                                'px-4 justify-between mt-1 group flex items-center py-2 text-sm leading-5 rounded-sm hover:text-white text-slate-500 focus:outline-none focus:text-gray-50 transition ease-in-out duration-150',
                            ...
                            >
                              <div className="flex items-center space-x-5">
                                {menuItem.icon !== undefined && (
                                  <SidebarIcon className="h-5 w-5 " icon={menuItem.icon} />
                                )}
                                <div>{menuItem.title}</div>
                              </div>
                            </Link>
                        ...
                        return (
                          <div>
                            ...
                            >
                              <div className="flex items-center space-x-5">
                                {menuItem.icon !== undefined && (
                                  <SidebarIcon className="h-5 w-5 text-gray-400" icon={menuItem.icon} />
                                )}
                                <div>{menuItem.title}</div>
                              </div>
                              {/*Expanded: "text-gray-400 rotate-90", Collapsed: "text-slate-200" */}
